...
-   *Wonder*: What am I looking at here? What do the numbers mean?

-   *Wonder*: What questions were being asked in "q1" through "q10"?

-   *Notice*: There are some wide variations in the "time spent"
    variable.

-   *Wonder*: Any connection between "time spent" and "FinalGradeCEMS"?
    Also what's going on with "points possible", "points earned" and
    "FinalGrade"?
...
-   Rows typically represent individual responses to a survey, or
    individual students.

What columns typically (or you think may) represent:

-   Columns typically represent response values to a survey, or values
    of a demographic variable, or a text response to an open-ended
    survey question.
...
```{r}
d |> 
  select(student_id, total_points_possible, total_points_earned, TimeSpent, FinalGradeCEMS)
```
...
-   The number of students with a final grade of 85 or higher is 279,
    considerably less than the 438 for a 70.

-   Seems like the range of courses is smaller - so those who had a
    final grade of 85+ are coming from a small subset of the total range
    of classes.
...
# Question: If you just look at the students with a final grade over 85, 
# how much time are they spending?
# IDK what any of these variables mean though LOL

d |> 
  select(TimeSpent_hours, FinalGradeCEMS) |>
  filter(FinalGradeCEMS > 85) |>
  arrange(desc(TimeSpent_hours))
```
...
# Question: Is there a correlation between low grades and time spent? 

d |> 
  filter(FinalGradeCEMS < 60) |>
ggplot(aes(x = FinalGradeCEMS, y = TimeSpent_hours)) +
  geom_point()
```
